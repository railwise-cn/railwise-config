...
| `oh-my-railwise.json` | oh-my-railwise 插件配置 | 项目根目录 `.railwise/oh-my-railwise.json` |
...
curl -o .railwise/oh-my-railwise.json https://raw.githubusercontent.com/railwise-cn/railwise-config/main/oh-my-railwise.json
```
...
## oh-my-railwise.json 详解

这是 [oh-my-railwise](https://github.com/railwise-cn/oh-my-railwise) 插件的配置文件，用于给 RAILWISE-CLI 的内置子代理（subagent）分配不同的模型。
...
```json
{
  "$schema": "https://raw.githubusercontent.com/railwise-cn/oh-my-railwise/main/assets/oh-my-railwise.schema.json",
  "agents": {
    "agent-name": {
...
**oh-my-railwise.json**（本仓库提供的默认配置）:
```json
{
  "agents": {
    "librarian": { "model": "zhipuai/glm-4-flash-250414" },
...
### Q: oh-my-railwise.json 中的子代理和 .railwise/agent/ 中的智能体有什么区别？

两者完全独立：

- `.railwise/agent/*.md` 中的 7 个智能体是**业务层**的领域专家（项目总工、方案设计师等），直接面向用户
- `oh-my-railwise.json` 中的 6 个子代理是**系统层**的后台助手（代码搜索、文档查找等），由系统自动调用
...
- [oh-my-railwise 插件](https://github.com/railwise-cn/oh-my-railwise)
